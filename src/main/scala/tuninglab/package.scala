...
package object tuninglab {
  ...
  def httpclient(host: String = "127.0.0.1", port: Int = 80, inBuffSz: Int = 8192)(outproc: (PrintWriter) => Any, inproc: (BufferedReader) => String = readall) = {
  ...
  def consume(zzz: String = "1m",
              depth: Int = 64,
              a: String = new String(Array.fill[Byte](1024)(0x0))): Int = {
    if (depth > 0) {
      consume(zzz, depth - 1) + 1
    } else {
      sleep(zzz)
      0
    }
